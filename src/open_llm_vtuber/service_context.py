...
class ServiceContext:
    ...
    async def load_from_config(self, config: Config) -> None:
        ...
        await self.init_agent(
            ...
            config.character_config.system_prompt
            or config.character_config.persona_prompt,
        )
    ...
    async def init_agent(self, agent_config: AgentConfig, prompt_base: str) -> None:
        ...
        if (
            ...
            and prompt_base
            == (
                self.character_config.system_prompt
                or self.character_config.persona_prompt
            )
        ):
            logger.debug("Agent already initialized with the same config.")
            return

        system_prompt = await self.construct_system_prompt(prompt_base)
