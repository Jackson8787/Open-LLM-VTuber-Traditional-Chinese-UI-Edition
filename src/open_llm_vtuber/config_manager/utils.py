...
def read_yaml_roundtrip(config_path: Union[str, Path]) -> Any:
    """Read YAML while preserving comments and formatting details."""
    config_file = Path(config_path)
...
def save_yaml_roundtrip(data: Any, config_path: Union[str, Path]) -> None:
    """Save YAML while preserving comments and formatting details."""
    config_file = Path(config_path)
    yaml_rt = YAML()
    yaml_rt.preserve_quotes = True
    yaml_rt.indent(mapping=2, sequence=4, offset=2)
