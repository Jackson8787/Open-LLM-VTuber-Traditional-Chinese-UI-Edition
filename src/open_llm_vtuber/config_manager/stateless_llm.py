...
class StatelessLLMBaseConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "interrupt_method": Description(
            ...
            zh="""用來表示中斷訊號的方法（提示詞模式）。如果 LLM 支援在聊天記憶中的任何位置插入系統提示詞，請使用「system」。
            否則請使用「user」。你通常不需要修改這個設定。""",
        ),
    }
...
class StatelessLLMWithTemplate(StatelessLLMBaseConfig):
    ...
    _OPENAI_COMPATIBLE_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "base_url": Description(en="Base URL for the API endpoint", zh="API 端點的基礎 URL"),
        "llm_api_key": Description(en="API key for authentication", zh="API 驗證金鑰"),
        "organization_id": Description(
            en="Organization ID for the API (Optional)", zh="組織 ID（可選）"
        ),
        "project_id": Description(
            en="Project ID for the API (Optional)", zh="專案 ID（可選）"
        ),
        "model": Description(en="Name of the LLM model to use", zh="LLM 模型名稱"),
        "temperature": Description(
            en="What sampling temperature to use, between 0 and 2.",
            zh="使用的取樣溫度，介於 0 到 2 之間。",
        ),
    }
...
class OllamaConfig(OpenAICompatibleConfig):
    ...
    _OLLAMA_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "llm_api_key": Description(
            ...
            zh="API 驗證金鑰（Ollama 預設為 'default_api_key'）",
        ),
        "keep_alive": Description(
            ...
            zh="最後一個請求後持續保留模型載入狀態的秒數。設為 -1 表示無限期保留。",
        ),
...
class ClaudeConfig(StatelessLLMBaseConfig):
    ...
    _CLAUDE_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "base_url": Description(
            en="Base URL for Claude API", zh="Claude API 的 API 端點"
        ),
        "llm_api_key": Description(en="API key for authentication", zh="API 驗證金鑰"),
        "model": Description(
            en="Name of the Claude model to use", zh="要使用的 Claude 模型名稱"
        ),
    }
...
class LlamaCppConfig(StatelessLLMBaseConfig):
    ...
    _LLAMA_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "model_path": Description(
            en="Path to the GGUF model file", zh="GGUF 模型檔案路徑"
        ),
    }
...
class StatelessLLMConfigs(I18nMixin, BaseModel):
    ...
    DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "stateless_llm_with_template": Description(
            en="Stateless LLM with Template", zh="帶模板的無狀態 LLM"
        ),
        "openai_compatible_llm": Description(
            en="Configuration for OpenAI-compatible LLM providers",
            zh="OpenAI 相容語言模型提供者設定",
        ),
        "ollama_llm": Description(en="Configuration for Ollama", zh="Ollama 設定"),
        "lmstudio_llm": Description(
            en="Configuration for LM Studio", zh="LM Studio 設定"
        ),
        "openai_llm": Description(
            en="Configuration for Official OpenAI API", zh="官方 OpenAI API 設定"
        ),
        "gemini_llm": Description(
            en="Configuration for Gemini API", zh="Gemini API 設定"
        ),
        "mistral_llm": Description(
            en="Configuration for Mistral API", zh="Mistral API 設定"
        ),
        "zhipu_llm": Description(en="Configuration for Zhipu API", zh="Zhipu API 設定"),
        "deepseek_llm": Description(
            en="Configuration for Deepseek API", zh="Deepseek API 設定"
        ),
        "groq_llm": Description(en="Configuration for Groq API", zh="Groq API 設定"),
        "claude_llm": Description(
            en="Configuration for Claude API", zh="Claude API 設定"
        ),
        "llama_cpp_llm": Description(
            en="Configuration for local Llama.cpp", zh="本機 Llama.cpp 設定"
        ),
    }
