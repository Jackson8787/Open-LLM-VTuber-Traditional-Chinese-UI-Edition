...
def check_frontend_submodule(lang=None):
    ...
    if not frontend_path.exists():
        if lang == "zh":
            logger.warning("未找到前端子模組，正在嘗試初始化子模組...")
        else:
            logger.warning(
        ...
        try:
            ...
            if frontend_path.exists():
                if lang == "zh":
                    logger.info("👍 前端子模組（和其他子模組）初始化成功。")
                else:
                    logger.info(
                ...
                if lang == "zh":
                    logger.critical(
                        '子模組初始化失敗。\n之後你可能會在瀏覽器中看到 {{"detail":"Not Found"}} 的錯誤提示。請檢查我們的快速入門指南與常見問題頁面，以取得更多資訊。'
                    )
                    logger.error(
                        "初始化子模組後，前端檔案仍然缺失。\n"
                        + "你是否手動變更或刪除了 `frontend` 資料夾？\n"
                        + "它是 Git 子模組，你不應該直接修改它。\n"
                        + "如果你這樣做了，請使用 `git restore frontend` 捨棄變更後再試一次。\n"
                    )
                else:
                    logger.critical(
            ...
            if lang == "zh":
                logger.critical(
                    f'子模組初始化失敗：{e}。\n你與 GitHub 之間可能有網路問題。之後你可能會在瀏覽器中看到 {{"detail":"Not Found"}} 的錯誤提示。請檢查我們的快速入門指南與常見問題頁面，以取得更多資訊。\n'
                )
            else:
                logger.critical(
